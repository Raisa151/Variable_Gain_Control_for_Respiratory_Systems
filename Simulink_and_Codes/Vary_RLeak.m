...
R_lung = 5/1000;
C_lung = 20;
R_leak = [60/800 60/1000 60/2000];
R_hose = 4.5/1000;
wn = 2*pi*30;

for i = 1:length(R_leak)
    A_h(i) = -(1/R_hose + 1/R_leak(i))/(R_lung*C_lung*(1/R_lung+1/R_hose+1/R_leak(i)));
    B_h(i) = (1/R_hose)/(R_lung*C_lung*(1/R_lung+1/R_hose+1/R_leak(i)));
    C_h(:,i) = [( 1/R_lung/(1/R_lung+1/R_hose+1/R_leak(i))),  -(1/R_hose+1/R_leak(i))/(R_lung*(1/R_lung+1/R_hose+1/R_leak(i)))]';
    D_h(:,i) = [ (1/R_hose/(1/R_lung+1/R_hose+1/R_leak(i))) , (1/R_hose / (R_lung*(1/R_lung+1/R_hose+1/R_leak(i))))]';
end
%%
s = tf("s");
%Blower B_s
...
for i = 1:3
    ...
    fprintf("Patient hose transfer function for R_leak %f is: \n",R_leak(i))
    H_s(:,i)
    
    fprintf("Overall plant transfer function for R_leak %f is  is : \n",R_leak(i))
    B_s*H_s(:,i)
end
...
xlabel("Time[s]")
ylabel("Paw[mbar]")
title("Effect of Varying RLeak on Paw")
for i = 1:length(R_leak)
...
for i = 1:length(R_leak)
    ...
    xlabel("Time[s]")
    ylabel("Qpat[mL/min]")
    title("Effect of Varying RLeak on Qpat")
    hold on
end
grid on
hold off